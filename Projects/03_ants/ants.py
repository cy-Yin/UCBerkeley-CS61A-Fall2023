...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        nearest = self.place
        distance = 0
        while nearest.is_hive == False:
            if nearest.bees != [] and \
                    self.lower_bound <= distance and self.upper_bound >= distance:
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        """
        A HungryAnt will eat bee in its place every time it takes an action 
        when it is not chewing.
        """
        current_place = self.place
        if current_place.bees != [] and \
                self.turns_to_chew == 0:
            self.eat_bee(random_bee(current_place.bees))
...
class TankAnt(ContainerAnt):
    '''TankAnt protects an ant in its place and 
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        current_place = self.place
        if current_place.ant == None or current_place.ant.blocks_path == False:
            return False
...
class LaserAnt(ThrowerAnt):
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        damage_now = self.basic_damage - distance * 0.25 - self.insects_shot * 0.0625
        if damage_now <= 0:
            return 0
        return damage_now
        # END Problem Optional 2
