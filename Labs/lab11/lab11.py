...
def floor_div(args):
    ...
    if divisors == nil:
        ...
        return floor_div(Pair(dividend, divisors.rest))
...
def eval_and(expressions):
    ...
    if expressions == nil:
        return scheme_t
    elif calc_eval(expressions.first) is scheme_f: # 0 == False but 0 is **not** False
                                                   # In Scheme only #f(scheme_f) is False
        return scheme_f
    else:
        if expressions.rest == nil:
            return calc_eval(expressions.first) # if all the operands equal to True, just return the last result
        else:
...
def eval_define(expressions):
    ...
    2
    """
    exp_key = expressions.first
    exp_binding = calc_eval(expressions.rest.first) # expressions.rest is a Pair but we just need the value
                                                    # For example eval_define(Pair("b", Pair(3, nil)))
                                                    # we just want to bind "b" to 3, not to Pair(3, nil)
                                                    # That's why we add calc_eval(expressions.rest.first) 
                                                    # rather than calc_eval(expresstions.rest) to the dictionary
    bindings[exp_key] = exp_binding
    return exp_key
    # BEGIN SOLUTION Q4
